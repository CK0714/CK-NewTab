...
![CK-Stars](https://img.shields.io/github/stars/CK0714/CK-NewTab?style=flat-square&color=gold)

**极简 | 纯净 | 零权限 | 符合 Chrome Manifest V3 规范**
自定义新标签页重定向扩展，只为个性化跳转，无任何冗余功能
...
3.  **完全合规**：严格遵循 Chrome Manifest V3 最新官方规范，CSP 安全策略合规
4.  **代码精简**：源码简洁清晰，无冗余逻辑，易于阅读
5.  **全内核兼容**：适配 Chrome、Edge、Brave 等所有 Chromium 内核浏览器

## 📥 安装方法
### 本地手动加载（推荐）
1. 点击仓库右上角 **<> Code → Download ZIP** 一键下载项目完整压缩包
2. 解压下载好的压缩包
3. 打开 Chrome 浏览器，进入扩展管理页面 chrome://extensions/
4. 开启页面右上角**开发者模式**
5. 点击左上角**加载已解压的扩展程序**，选择解压后的项目文件夹
6. 加载完成即可正常使用
...
- 安全策略：全部脚本外部引用，无内联脚本，满足浏览器 CSP 安全策略
...
### 使用须知
1.  **合法合规**：本项目仅用于个人合法的浏览器扩展自定义使用，严禁用于任何非法、侵权或违反网络服务条款的行为。
2.  **非商业用途**：本项目完全免费，仅限个人学习、研究和非商业使用。
3.  **安全责任**：本扩展仅在本地浏览器环境运行，通过 `localStorage` 纯本地存储配置。
    开发者不对因使用本扩展导致的浏览器配置错乱、数据丢失或其他意外情况承担直接责任。
4.  **功能边界**：本扩展仅实现新标签页自定义跳转功能，全程本地运行，不收集、不上传任何用户隐私数据，不篡改浏览器核心功能。
5.  **用户自主责任**：用户需自行保证使用行为符合当地法律法规、浏览器服务协议，开发者不对用户违规使用行为承担责任。
